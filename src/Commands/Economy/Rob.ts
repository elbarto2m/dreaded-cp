...
export default class command extends BaseCommand {
    override execute = async (M: Message): Promise<void> => {
        ...
        const { wallet: Wallet, inventory } = await this.client.DB.getUser(target)
        await this.client.DB.user.set(`${M.sender.jid}.lastRob`, Date.now())
        if (inventory.findIndex((x) => x.item === 'protection charm') > -1) {
            await this.client.DB.useInventoryItem(target, 'protection charm')
            return void M.reply(
                `🎐 *@${target.split('@')[0]}'s* *Protection Charm* protected themselves from getting robbed.`,
                'text',
                undefined,
                undefined,
                undefined,
